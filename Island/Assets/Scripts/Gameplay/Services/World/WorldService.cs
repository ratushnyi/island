...
namespace Island.Gameplay.Services.World
{
    ...
    public class WorldService : ServiceBase, INetworkInitialize
    {
        ...
        {
            if (worldItemObject.Health == 0)
            {
                _worldProfile.ObjectHealthDictionary.Remove(worldItemObject.Hash);
            }

            if (!_worldProfile.ObjectHealthDictionary.TryAdd(worldItemObject.Hash, worldItemObject.Health))
            {
                _worldProfile.ObjectHealthDictionary[worldItemObject.Hash] = worldItemObject.Health;
            }
        }
